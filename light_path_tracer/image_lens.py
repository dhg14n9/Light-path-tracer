    ...
    if n_escaped > 0:
        ...
        if render_loop_around:
            ...
            if wrap_outside_background_plane:
                ...
                src_x = np.rint(src_x_cam * fx + width / 2).astype(np.intp)
                src_y = np.rint(src_y_cam * fy + height / 2).astype(np.intp)
                lensed[escaped] = source_image[src_y % height, src_x % width]
            else:
...
def main(metric=None, M=1.0, a=0.0, r_obs_mult=100.0,
    ...
    lensed_image = render_lensed_image(
        ...
        wrap_outside_background_plane=wrap_outside_background_plane,
        psi=psi,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=("Wrap rays that miss or fall behind the background plane back "
              "onto the image with modulo indexing and disable the winding-"
              "color and magenta miss fallbacks"),
    )
